...
from src.models import FineGrainedRetriever, LLMs, Retriever, BaselineRetriever
...
def main():
    ...
    if args.mode == "inference":
        test_set = RetrievalDataset(config=config["dataset"], split='test', training=False, domains=args.domains)
        # from torch.utils.data import random_split
        # test_set, _ = random_split(test_set, [250, len(test_set) - 250])
    ...
    print(len(train_set))
    train_loader = DataLoader(train_set, batch_size=16, shuffle=True, collate_fn=collate_fn, drop_last=False)
    ...
    for data in train_set:
        d = degree(data["graph"].edge_index[1], num_nodes=len(data['entities']), dtype=torch.long)
    ...
    ibtn = BaselineRetriever(config['retriever'], deg=deg.to(device)).to(device)
    if args.ckpt_path:
        ...
        try:
            ...
            new_state_dict = {}
            for k, v in warmup_ckpt['model_state_dict'].items():
                if k.startswith("PNA."):
                    new_key = k.replace("PNA.", "encoder.", 1)
                else:
                    new_key = k
                new_state_dict[new_key] = v
            ibtn.load_state_dict(new_state_dict)
