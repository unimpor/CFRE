...
def get_prompts(each_qa, mode, sys_prompt, cot_prompt, thres, seed=0):
    ...
    if 'rog' in mode:
        ...
        if 'rmhtLim' in mode:
            input_triplets = remove_same_head_tail(input_triplets[:num_sampled_triplets], 'rmht')
        elif 'rmht' in mode:
            input_triplets = remove_same_head_tail(input_triplets, mode)

        if 'mergeLim' in mode:
            input_triplets = merge_tuples(merge_tuples(input_triplets[:num_sampled_triplets], 0), 2)
        elif 'merge' in mode:
            input_triplets = merge_tuples(merge_tuples(input_triplets, 0), 2)
